...
When calling the function array of chars sized number of bits + 1 (for '\0') should be provided (char * arr) */
...
void convert_ascii_value_to_10_bit_binary(char c, char * arr) {
	if (!((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))) {
		fprintf(stderr, "ERROR: char is not an alphabetic character");
	}
	convert_dec_to_x_bit_binary(c, 11, arr);
}
